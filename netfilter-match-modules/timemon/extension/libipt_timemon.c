...
{
	printf("timemon options:\n");
	printf("  --id [unique identifier for querying time usage]\n");
	...
	printf("  --greater_than [SECONDS]\n");
	printf("  --less_than [SECONDS]\n");
	printf("  --current_timeusage [SECONDS]\n");
	...
	printf("  --tcheck Check another timemon rule without incrementing it\n");
	printf("  --tcheck_with_src_dst_swap Check another timemon rule without incrementing it, swapping src & dst ips for check\n");
}

static struct option opts[] = 
{
	{ .name = "id", 			.has_arg = 1, .flag = 0, .val = TIMEMON_ID },	
	{ .name = "type", 			.has_arg = 1, .flag = 0, .val = TIMEMON_TYPE },	
	{ .name = "subnet", 			.has_arg = 1, .flag = 0, .val = TIMEMON_SUBNET },	
	{ .name = "greater_than", 		.has_arg = 1, .flag = 0, .val = TIMEMON_GT },
	{ .name = "less_than", 			.has_arg = 1, .flag = 0, .val = TIMEMON_LT },	
	{ .name = "current_timeusage",		.has_arg = 1, .flag = 0, .val = TIMEMON_CURRENT },	
	{ .name = "reset_interval",		.has_arg = 1, .flag = 0, .val = TIMEMON_RESET_INTERVAL },
	{ .name = "reset_time",			.has_arg = 1, .flag = 0, .val = TIMEMON_RESET_TIME },
	{ .name = "last_backup_time",		.has_arg = 1, .flag = 0, .val = TIMEMON_LAST_BACKUP},
	{ .name = "tcheck",	 		.has_arg = 0, .flag = 0, .val = TIMEMON_CHECK_NOSWAP },
	{ .name = "tcheck_with_src_dst_swap",	.has_arg = 0, .flag = 0, .val = TIMEMON_CHECK_SWAP },
	{ .name = 0 }
};
...
{
	struct ipt_timemon_info *info = (struct ipt_timeinfo_info *)(*match)->data;
	int valid_arg = 0;
	long int num_read;
	uint64_t read_64;
	time_t read_time;
	...
	if(*flags == 0)
	{
		...
		info->type = TIMEMON_COMBINED;
		info->check_type = TIMEMON_CHECK_NOSWAP;
		info->local_subnet = 0;
		info->local_subnet_mask = 0;
		info->cmp = TIMEMON_MONITOR; /* don't test greater/less than, just monitor timeusage */
		info->current_timeusage = 0;
		info->reset_is_constant_interval = 0;
		info->reset_interval = TIMEMON_NEVER;
		info->reset_time=0;
		...
		*flags = *flags + TIMEMON_INITIALIZED;
	}

	switch (c)
	{
		case TIMEMON_ID:
			if(strlen(optarg) < TIMEMON_MAX_ID_LENGTH)
			{
		...
		case TIMEMON_TYPE:
			...
			{
				info->type = TIMEMON_INDIVIDUAL_SRC;
			}
			else if(strcmp(optarg, "individual_dst") == 0)
			{
				info->type = TIMEMON_INDIVIDUAL_DST;
			}
			else if(strcmp(optarg, "individual_local") == 0)
			{
				info->type = TIMEMON_INDIVIDUAL_LOCAL;
				*flags = *flags + TIMEMON_REQUIRES_SUBNET;
			}
			else if(strcmp(optarg, "individual_remote") == 0)
			{
				info->type = TIMEMON_INDIVIDUAL_REMOTE;
				*flags = *flags + TIMEMON_REQUIRES_SUBNET;
			}
			else
			{
		...
		case TIMEMON_LT:
			...
			if(num_read > 0 && (*flags & TIMEMON_CMP) == 0)
			{
				info->cmp = TIMEMON_LT;
				info->timeusage_cutoff = read_64;
				valid_arg = 1;
			}
			c = TIMEMON_CMP; //only need one flag for less_than/greater_than
			break;
		case TIMEMON_GT:
			num_read = sscanf(argv[optind-1], "%lld", &read_64);
			if(num_read > 0  && (*flags & TIMEMON_CMP) == 0)
			...
			c = TIMEMON_CMP; //only need one flag for less_than/greater_than
			break;
		case TIMEMON_CHECK_NOSWAP:
			if(  (*flags & TIMEMON_CMP) == 0 )
			{
				info->cmp = TIMEMON_CHECK;
				info->check_type = TIMEMON_CHECK_NOSWAP;
				valid_arg = 1;
			}
			c = TIMEMON_CMP;
			break;
		case TIMEMON_CHECK_SWAP:
			if(  (*flags & TIMEMON_CMP) == 0 )
			{
				info->cmp = TIMEMON_CHECK;
				info->check_type = TIMEMON_CHECK_SWAP;
				valid_arg = 1;
			}
			c = TIMEMON_CMP;
			break;
		case TIMEMON_CURRENT:
			num_read = sscanf(argv[optind-1], "%lld", &read_64);
			if(num_read > 0 )
			{
				info->current_timeusage = read_64;
				valid_arg = 1;
			}
			break;
		case TIMEMON_RESET_INTERVAL:
			valid_arg = 1;
			if(strcmp(argv[optind-1],"minute") ==0)
			{
				info->reset_interval = TIMEMON_MINUTE;
				info->reset_is_constant_interval = 0;
			}
			else if(strcmp(argv[optind-1],"hour") ==0)
			{
				info->reset_interval = TIMEMON_HOUR;
				info->reset_is_constant_interval = 0;
			}
			else if(strcmp(argv[optind-1],"day") ==0)
			{
				info->reset_interval = TIMEMON_DAY;
				info->reset_is_constant_interval = 0;
			}
			else if(strcmp(argv[optind-1],"week") ==0)
			{
				info->reset_interval = TIMEMON_WEEK;
				info->reset_is_constant_interval = 0;
			}
			else if(strcmp(argv[optind-1],"month") ==0)
			{
				info->reset_interval = TIMEMON_MONTH;
				info->reset_is_constant_interval = 0;
			}
	...
	*flags = *flags + (unsigned int)c;
	...
	if((*flags & TIMEMON_RESET_TIME) == TIMEMON_RESET_TIME && (*flags & TIMEMON_RESET_INTERVAL) == TIMEMON_RESET_INTERVAL)
	{
		if(	(info->reset_interval == TIMEMON_NEVER) ||
			(info->reset_interval == TIMEMON_MONTH && info->reset_time >= 60*60*24*28) ||
			(info->reset_interval == TIMEMON_WEEK && info->reset_time >= 60*60*24*7) ||
			(info->reset_interval == TIMEMON_DAY && info->reset_time >= 60*60*24) ||
			(info->reset_interval == TIMEMON_HOUR && info->reset_time >= 60*60) ||
			(info->reset_interval == TIMEMON_MINUTE && info->reset_time >= 60) 
		  )
		{
			valid_arg = 0;
	...
	if(info->type != TIMEMON_COMBINED && (*flags & TIMEMON_CURRENT) == TIMEMON_CURRENT)
	{
		valid_arg = 0;
		param_problem_exit_error("You may only specify current timemon for combined type\n  Use user-space library for setting timemon for individual types");
	}
...
static void print_timemon_args( struct ipt_timemon_info* info )
{
	if(info->cmp == TIMEMON_CHECK)
	{
		if(info->check_type == TIMEMON_CHECK_NOSWAP)
		{
			printf("--tcheck ");
		}
		else
		{
			printf("--tcheck_with_src_dst_swap ");
		}
	}
	printf("--id %s ", info->id);
	...
	if(info->cmp != TIMEMON_CHECK)
	{
		...
		if(info->type == TIMEMON_INDIVIDUAL_DST)
		{
		...
		if(info->type == TIMEMON_INDIVIDUAL_REMOTE)
		{
		...
		{
			printf("--greater_than %lld ", info->timeusage_cutoff);
		}
		if(info->cmp == TIMEMON_LT)
		{
			printf("--less_than %lld ", info->timeusage_cutoff);
		}
		if (info->type == TIMEMON_COMBINED) /* too much data to print for multi types, have to use socket to get/set data */
		{
			if( info->reset_interval != TIMEMON_NEVER && info->next_reset != 0 && info->next_reset < now)
			{
				/* 
				 * current timemon only gets reset when first packet after reset interval arrives, so output
				 * zero if we're already past interval, but no packets have arrived 
				 */
				printf("--current_timeusage 0 ");
			}
			else 
			{
				printf("--current_timeusage %lld ", info->current_timeusage);
			}
		}
...
static void final_check(unsigned int flags)
{
	...
	if( (flags & TIMEMON_RESET_INTERVAL) == 0 && (flags & TIMEMON_RESET_TIME) != 0)
	{
		param_problem_exit_error("You may not specify '--reset_time' without '--reset_interval' ");
	}
	if( (flags & TIMEMON_REQUIRES_SUBNET) == TIMEMON_REQUIRES_SUBNET && (flags & TIMEMON_SUBNET) == 0 )
	{
...
{
	printf("timemon ");
	struct ipt_timemon_info *info = (struct ipt_timemon_info *)match->data;

	print_timemon_args(info);
}
...
static struct iptables_match timemon = 
{ 
	.next		= NULL,
 	.name		= "timemon",
	#ifdef XTABLES_VERSION_CODE
	...
	#endif
	.size		= XT_ALIGN(sizeof(struct ipt_timemon_info)),
	.userspacesize	= XT_ALIGN(sizeof(struct ipt_timemon_info)),
	.help		= &help,
	.parse		= &parse,
