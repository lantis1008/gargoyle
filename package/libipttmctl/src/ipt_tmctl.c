...
static int timemon_semaphore = -1;
...
static int get_timemon_data(			char* id, 
						unsigned char get_history, 
						char* ip, 
...
static int set_timemon_data(			char* id, 
...
{
	int locked = 0;
	if(timemon_semaphore == -1)
	{
		get_sem(&timemon_semaphore, (key_t)(TIMEMON_SEMAPHORE_KEY) );
	}
	if(timemon_semaphore != -1)
	{
		do
		{
			locked = lock_sem(timemon_semaphore);
...
static int unlock(void)
{
	...
	if(timemon_semaphore != -1)
	{
		unlocked = unlock_sem(timemon_semaphore);
	}
						...
						)
{
	...
	else
	{
		while(next + reset_time <=  current_start_time)
		{
			...
			curr->tm_isdst = -1;
			if(reset_interval == TIMEMON_MINUTE)
			{
				curr->tm_sec = 0;
				curr->tm_min = curr->tm_min+1;
				next = mktime(curr);
			}
			else if(reset_interval == TIMEMON_HOUR)
			{
				curr->tm_sec  = 0;
				curr->tm_min  = 0;
			...
			else if(reset_interval == TIMEMON_DAY)
			{
				curr->tm_sec  = 0;
				curr->tm_min  = 0;
				curr->tm_hour = 0;
			...
			else if(reset_interval == TIMEMON_WEEK)
			{
				curr->tm_sec  = 0;
				curr->tm_min  = 0;
				curr->tm_hour = 0;
			...
			else if(reset_interval == TIMEMON_MONTH)
			{
				curr->tm_sec  = 0;
				curr->tm_min  = 0;
				curr->tm_hour = 0;
					...
					)
{
    ...
    ip_tm_kernel_data_item* ip_tm_data = (ip_tm_kernel_data*)(in_buffer + *in_index);
	if(get_history == 0)
	{
		(((ip_tm*)out_data)[*out_index]).ip = ip;
		*in_index = *in_index + 4;
		(((ip_tm*)out_data)[*out_index]).tm = *( (uint64_t*)(in_buffer + *in_index) );
		*in_index = *in_index + 8;
	}
	else
	{
			ip_tm_history *history = ((ip_tm_history*)out_data) + *out_index;
			history->reset_interval = reset_interval;
			history->reset_time = reset_time;
			history->is_constant_interval = is_constant_interval;

            history->ip = ip_tm_data->ip;
			history->num_nodes = ip_tm_data->num_nodes;
			history->first_start = ip_tm_data->first_start;
			history->first_end   = ip_tm_data->first_end;
			history->last_end    = ip_tm_data->last_end;

			history->history_tms =  (uint64_t*)malloc( (history->num_nodes+1)*sizeof(uint64_t) );
            ...
            *in_index += 32;
			...
			{
                *in_index += 8;
				(history->history_tms)[node_index] =  ip_tm_data->iptm_data[node_index];
			}
...
static int get_timemon_data(char* id, unsigned char get_history, char* ip, unsigned long* num_ips, void** data, unsigned long max_wait_milliseconds)
{	
	
	unsigned char buf[TIMEMON_QUERY_LENGTH];
	memset(buf, '\0',  TIMEMON_QUERY_LENGTH);
	int done = 0;
    ip_tm_kernel_data* ip_tm_data = (ip_tm_kernel_data*)(buf);
	...
	*request_history = get_history;
	...
	{
		uint32_t size = TIMEMON_QUERY_LENGTH;
	
		getsockopt(sockfd, IPPROTO_IP, TIMEMON_GET, buf, &size);
		...
		{
			uint32_t total_ips                 = ip_tm_data->ip_total;
			/*uint32_t next_ip_index             = *( (uint32_t*)(buf+5) ); //unused */
			uint32_t response_ips              = ip_tm_data->ip_num;
			time_t reset_interval              = ip_tm_data->reset_interval;
			time_t reset_time                  = ip_tm_data->reset_time;
			unsigned char is_constant_interval = ip_tm_data->reset_is_constant_interval;
			...
			if(!data_initialized)
			{
				*num_ips = total_ips;
				if(get_history)
				{
					*data = (void*)malloc(sizeof(ip_tm_history)*(total_ips+1));
					memset(*data, 0, sizeof(ip_tm_history)*(total_ips+1));
				}
				else
				{
					*data = (void*)malloc(sizeof(ip_tm)*(total_ips+1));
					memset(*data, 0, sizeof(ip_tm)*(total_ips+1));
				}
			}
	...
	if( (error != 0) && data_initialized)
	{
		if(get_history)
		{
			free_ip_tm_histories(  (ip_tm_history*)(*data), *num_ips );
		}
		else
		{
			free(*data);
...
static int set_ip_block(void* ip_block_data, unsigned char is_history, unsigned char* output_buffer, uint32_t* current_output_index, uint32_t output_buffer_length)
{
	if(is_history)
	{
		ip_tm_history* history = (ip_tm_history*)ip_block_data;
		...
		*current_output_index = *current_output_index + 8;
		...
		{
			*( (uint64_t*)(output_buffer + *current_output_index) ) = (history->history_tms)[ node_num ];
			*current_output_index = *current_output_index + 8;
		}
	}
	else
	{
		...
		ip_tm* ib = (ip_tm*)ip_block_data;
		*( (uint32_t*)(output_buffer + *current_output_index) ) = ib->ip;
		*current_output_index = *current_output_index + 4;
		*( (uint64_t*)(output_buffer + *current_output_index) ) = ib->tm;
...
static int set_timemon_data(char* id, unsigned char zero_unset, unsigned char set_history, unsigned long num_ips, time_t last_backup, void* data, unsigned long max_wait_milliseconds)
{
	unsigned char buf[TIMEMON_QUERY_LENGTH];
	memset(buf, 0, TIMEMON_QUERY_LENGTH);
	int done = 0;
	...
	*last_backup_time = (uint64_t)last_backup;
	memcpy(set_id, id, TIMEMON_MAX_ID_LENGTH);
	set_id[TIMEMON_MAX_ID_LENGTH-1] = '\0';

	while(!done && sockfd >= 0 && got_lock)
	{
		uint32_t buf_index = (3*4) + (2*1) + 8 + TIMEMON_MAX_ID_LENGTH;
		uint32_t ip_index = *next_ip_index;
		unsigned char buffer_full = 0;
		memset( (buf + buf_index), 0, (TIMEMON_QUERY_LENGTH-buf_index) );
		*num_ips_in_buffer = 0;
		...
		{
			void *next_data = set_history ? (void*)(((ip_tm_history*)data) + ip_index) : (void*)(((ip_tm*)data) + ip_index);
			buffer_full = set_ip_block(next_data , set_history, buf, &buf_index, TIMEMON_QUERY_LENGTH);
			ip_index = buffer_full ? ip_index : ip_index+1;
			*num_ips_in_buffer = buffer_full ? *num_ips_in_buffer : *num_ips_in_buffer + 1;
			done = (ip_index >= *total_ips);

		}
		setsockopt(sockfd, IPPROTO_IP, TIMEMON_SET, buf, TIMEMON_QUERY_LENGTH);
...
void free_ip_tm_histories(ip_tm_history* histories, int num_histories)
{
	if(histories == NULL)
	{
		return;
	...
	{
		if((histories[history_index]).history_tms != NULL)
		{
			free( (histories[history_index]).history_tms );
		}
	}
	free(histories);
}
...
int get_all_timemon_history_for_rule_id(char* id, unsigned long* num_ips, ip_tm_history** data, unsigned long max_wait_milliseconds)
{
	return get_timemon_data(id, 1, "ALL", num_ips, (void*)data, max_wait_milliseconds);
}
int get_ip_timemon_history_for_rule_id(char* id, char* ip, ip_tm_history** data, unsigned long max_wait_milliseconds)
{
	unsigned long num_ips;
	return get_timemon_data(id, 1, ip, &num_ips, (void*)data, max_wait_milliseconds);
}
int get_all_timemon_usage_for_rule_id(char* id, unsigned long* num_ips, ip_tm** data, unsigned long max_wait_milliseconds)
{
	return get_timemon_data(id, 0, "ALL", num_ips, (void*)data, max_wait_milliseconds);
}
int get_ip_timemon_usage_for_rule_id(char* id,  char* ip, ip_tm** data, unsigned long max_wait_milliseconds)
{
	unsigned long num_ips;
	return get_timemon_data(id, 0, ip, &num_ips, (void*)data, max_wait_milliseconds);
}


int set_timemon_history_for_rule_id(char* id, unsigned char zero_unset, unsigned long num_ips, ip_tm_history* data, unsigned long max_wait_milliseconds)
{
	return set_timemon_data(id, zero_unset, 1, num_ips, 0, data, max_wait_milliseconds);
}

int set_timemon_usage_for_rule_id(char* id, unsigned char zero_unset, unsigned long num_ips, time_t last_backup, ip_tm* data, unsigned long max_wait_milliseconds)
{
	return set_timemon_data(id, zero_unset, 0, num_ips, last_backup, data, max_wait_milliseconds);
}
...
int save_usage_to_file(ip_tm* data, unsigned long num_ips, char* out_file_path)
{
	...
	if(out_file != NULL)
	{
		...
		for(out_index=0; out_index < num_ips; out_index++)
		{
			...
			ip_tm next = data[out_index];
			ipaddr.s_addr = next.ip;
			fprintf(out_file, "%-15s\t%lld\n", inet_ntoa(ipaddr), (long long int)next.tm);
		}
		fclose(out_file);
		success = 1;
	}
...
int save_history_to_file(ip_tm_history* data, unsigned long num_ips, char* out_file_path)
{
	...
	if(out_file != NULL)
	{
		...
		unsigned char  tm_fits_in_32bits = 1;
		uint32_t out_index=0;
		for(out_index=0; out_index < num_ips && tm_fits_in_32bits; out_index++)
		{
			uint32_t node_index = 0;
			ip_tm_history next = data[out_index];
			for(node_index=0; node_index < next.num_nodes && tm_fits_in_32bits; node_index++)
			{
				uint64_t tm = (next.history_tms)[node_index];
				tm_fits_in_32bits = tm_fits_in_32bits && (tm <  INT32_MAX);
			}
		}
		...
		for(out_index=0; out_index < num_ips; out_index++)
		{
			...
			{
				uint64_t dummy = 0;
				unsigned char tm_bits = 32;
				fwrite( &dummy, 8, 1, out_file);
				fwrite( &dummy, 8, 1, out_file);
				fwrite( &dummy, 8, 1, out_file);
				fwrite( &tm_bits,  1, 1, out_file);
			}
			else
			{
				uint32_t node_index = 0;
				...
				unsigned char  tm_bits = 32;
				for(node_index=0; node_index < next.num_nodes && tm_bits == 32; node_index++)
				{
					uint64_t tm = (next.history_tms)[node_index];
					tm_bits = tm_bits == 32 && (tm <  INT32_MAX) ? 32 : 64;
				}
				...
				for(node_index=0; node_index < next.num_nodes; node_index++)
				{
					if(tm_bits == 32)
					{
						uint32_t tm = (uint32_t)(next.history_tms)[node_index];
						fwrite( &tm, 4, 1, out_file);

					}
					else
					{
						uint64_t tm = (next.history_tms)[node_index];
						fwrite( &tm, 8, 1, out_file);
					}
				}
			}
		}
	...
	*last_backup = 0;
	...
	if(in_file != NULL)
	{
		...
		*num_ips = (num_data_parts/2) + 1;
       		data = (ip_tm*)malloc( (*num_ips) * sizeof(ip_tm) );
		*num_ips = 0;
		...
		while(data_part_index < num_data_parts)
		{
			...
			if(valid && data_index < num_data_parts)
			{
				...
				valid = sscanf(data_parts[data_part_index], "%lld", (long long int*)&(next.tm) );
				data_part_index++;
			}
			else
			{
	...
	if(in_file != NULL)
	{
		...
		*num_ips = (unsigned long)nips;
		...
		if(*num_ips > 0)
		{
			...
			data = (ip_tm_history*)malloc( (*num_ips) * sizeof(ip_tm_history));
		}
		...
		for(ip_index=0; ip_index < *num_ips; ip_index++)
		{
			...
			next.history_tms          = NULL;
			if(next.num_nodes > 0)
			{
				next.history_tms = malloc( next.num_nodes * sizeof(uint64_t) );
				uint32_t node_index = 0;
				for(node_index=0; node_index < next.num_nodes; node_index++)
				{
					if(tm_bits == 32)
					{
						uint32_t nexttm = 0;
						fread(&nexttm, 4, 1, in_file);
						(next.history_tms)[node_index] = (uint64_t)nexttm;

					}
					else
					{
						uint64_t nexttm = 0;
						fread(&nexttm, 8, 1, in_file);
						(next.history_tms)[node_index] = nexttm;
					}
				}
			}
			data[ip_index] = next;
...
void print_usage(FILE* out, ip_tm* usage, unsigned long num_ips)
{
	...
	for(usage_index =0; usage_index < num_ips; usage_index++)
	{
		...
		{
			fprintf(out, "%-15s\t%lld\n", "COMBINED", (long long int)next.tm);
		}
	}
	fprintf(out, "\n");
}
...
void print_histories(FILE* out, char* id, ip_tm_history* histories, unsigned long num_histories, char output_type)
{
	...
	for(history_index=0; history_index < num_histories; history_index++)
	{
		ip_tm_history history = histories[history_index];
		...
		if(history_initialized)
		{
			...
			{
				uint64_t tm = (history.history_tms)[hindex];
				if(output_type == 'm')
				{
					if(hindex != 0) { printf(","); };
					printf("%lld", (unsigned long long int)tm);
				}
				else if(times != NULL)
				{
					...
					if(output_type == 'h')
					{
						fprintf(out, "%lld\t%s\t%s\n", (unsigned long long int)tm, start_str, end_str);
					}
					else
					{
						fprintf(out, "%s,%s,%ld,%ld,%lld\n", id, ip_str, start, end, (unsigned long long int)tm );
					}
...
void unlock_timemon_semaphore(void)
{
	unlock();
}
...
void signal_handler(int sig)
{
	...
	{
		unlock_timemon_semaphore();
		exit(0);
	}
}

void unlock_timemon_semaphore_on_exit(void)
{
