...
int main(int argc, char **argv)
{
	...
	{	
		switch(c)
		{
			...
			case 'U':
			default:
				fprintf(stderr, "USAGE:\n\t%s -i [ID] -b [LAST_BACKUP_TIME] -f [IN_FILE_NAME] [ IP TIMEMON PAIRS, IF -f NOT SPECIFIED ]\n", argv[0]);
				exit(0);
		}
	}
	...
	else
	{
		...
		unsigned long num_ips = num_data_parts/2;
   		ip_tm* buffer = (ip_tm*)malloc(num_ips*sizeof(ip_tm));
		...
		while(data_index < num_data_parts)
		{
			...
			if(valid && data_index < num_data_parts)
			{
				...
				valid = sscanf(data_parts[data_index], "%lld", (long long int*)&(next.tm) );
				data_index++;
			}
			else
			{
		...
		query_succeeded = set_timemon_usage_for_rule_id(id, 1, num_ips, last_backup, buffer, 1000);
	}
