...
int main(int argc, char **argv)
{
	...
	{	
		switch(c)
		{
			case 'i':
			case 'I':
				if(strlen(optarg) < TIMEMON_MAX_ID_LENGTH && strlen(optarg) > 0)
				{
					id = strdup(optarg);
				}
				else
	...
	if(get_history == 0)
	{
		if(address == NULL)
		{
			query_succeeded = get_all_timemon_usage_for_rule_id(id, &num_ips, (ip_tm**)&ip_buf, 1000);
		}
		else
		{
			num_ips = 1;
			query_succeeded = get_ip_timemon_usage_for_rule_id(id, address, (ip_tm**)&ip_buf, 1000);
		}
	}
	else
	{
		if(address == NULL)
		{
			query_succeeded = get_all_timemon_history_for_rule_id(id, &num_ips, (ip_tm_history**)&ip_buf, 1000);
		}
		else
		{
			num_ips = 1;
			query_succeeded = get_ip_timemon_history_for_rule_id(id, address, (ip_tm_history**)&ip_buf, 1000);
		}
	}
	if(!query_succeeded)
	{
		fprintf(stderr, "ERROR: Timemon query failed, make sure rule with specified id exists, and that you are performing only one query at a time.\n\n");
		exit(0);
	}
