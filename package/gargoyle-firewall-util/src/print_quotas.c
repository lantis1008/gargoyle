/*  print_quotas --	Used to print quota data from iptables rules that use the "bandwidth" and "timemon" modules
...
int main(void)
{
	...
	while(quota_sections->length > 0)
	{
		...
		char* types[] = { "combined_limit", "ingress_limit", "egress_limit", "time_combined_limit" };
		char* prefixes[] = { "bw_", "bw_", "bw_", "tm_"};
		char* postfixes[] = { "_combined", "_ingress", "_egress", "_combined" };
			...
			for(ip_index=0; ip_index < num_ips; ip_index++)
			{
				...
				if(used != NULL)
				{
					...
					for(type_index=0; type_index < 4; type_index++)
					{
						...
						{
							printf("%s%lld", print_comma, (long long int)used[type_index+4]);
						}
						sprintf(print_comma, ", ");
					}
					printf(" ];\n");
